PvpMyQuizHelpPanel = PvpView:new{
	jsonFile = 'panel/pvp_my_quiz_help_panel.json',
	panelName = 'pvp-my-quiz-help-panel'
}

function PvpMyQuizHelpPanel:init()
	self.panel:setAdaptInfo('help_bg_img', 'help_img')
	self.panel:registerInitHandler(function()
		local root = self.panel:GetRawPanel()
		self:registerButtonWithHandler(root, 'close_btn', BUTTON_SOUND_TYPE.CLOSE_EFFECT, function()
			PvpController.close(self, ELF_HIDE.ZOOM_OUT_FADE_OUT)
		end)
		self:registerButtonWithHandler(root, 'help_bg_img', nil, function()
			PvpController.close(self, ELF_HIDE.ZOOM_OUT_FADE_OUT)
		end)

		local sv = tolua.cast(root:getChildByName('system_sv'), 'UIScrollView')
		local infoTx1 = tolua.cast(sv:getChildByName('info_1_tx'), 'UILabel')
		infoTx1:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC1'))
		infoTx1:setPreferredSize(580,1)
		local infoTx2 = tolua.cast(sv:getChildByName('info_2_tx'), 'UILabel')
		infoTx2:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC2'))
		infoTx2:setPreferredSize(580,1)
		local infoTx3 = tolua.cast(sv:getChildByName('info_3_tx'), 'UILabel')
		infoTx3:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC3'))
		infoTx3:setPreferredSize(580,1)
		local infoTx4 = tolua.cast(sv:getChildByName('info_4_tx'), 'UILabel')
		infoTx4:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC4'))
		infoTx4:setPreferredSize(580,1)	
		local infoTx5 = tolua.cast(sv:getChildByName('info_5_tx'), 'UILabel')
		infoTx5:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC5'))
		infoTx5:setPreferredSize(580,1)
		local infoTx6 = tolua.cast(sv:getChildByName('info_6_tx'), 'UILabel')
		infoTx6:setText(getLocalStringValue('E_STR_PVP_MY_QUIZ_HELP_DESC6'))
		infoTx6:setPreferredSize(580,1)	
		-- local infoTx7 = tolua.cast(sv:getChildByName('info_7_tx'), 'UILabel')
		-- infoTx7:setText(getLocalStringValue('E_STR_PVP_KNOCKOUT_HELP_DESC7'))
		-- infoTx7:setPreferredSize(580,0)	
		-- local infoTx8 = tolua.cast(sv:getChildByName('info_8_tx'), 'UILabel')
		-- infoTx8:setText(getLocalStringValue('E_STR_PVP_KNOCKOUT_HELP_DESC8'))
		-- infoTx8:setPreferredSize(580,0)	
		-- local infoTx9 = tolua.cast(sv:getChildByName('info_9_tx'), 'UILabel')
		-- infoTx9:setText(getLocalStringValue('E_STR_PVP_KNOCKOUT_HELP_DESC9'))
		-- infoTx9:setPreferredSize(580,0)	
		-- local infoTx10 = tolua.cast(sv:getChildByName('info_10_tx'), 'UILabel')
		-- infoTx10:setText(getLocalStringValue('HELP_LEGION_10'))
		-- infoTx10:setPreferredSize(580,0)	
		-- local infoTx11 = tolua.cast(sv:getChildByName('info_11_tx'), 'UILabel')
		-- infoTx11:setText(getLocalStringValue('HELP_LEGION_11'))
		-- infoTx11:setPreferredSize(580,0)	
		-- local infoTx12 = tolua.cast(sv:getChildByName('info_12_tx'), 'UILabel')
		-- infoTx12:setText(getLocalStringValue('HELP_LEGION_12'))
		-- infoTx12:setPreferredSize(580,0)	
		-- local infoTx13 = tolua.cast(sv:getChildByName('info_13_tx'), 'UILabel')
		-- infoTx13:setText(getLocalStringValue('HELP_LEGION_13'))
		-- infoTx13:setPreferredSize(580,0)	
		-- local infoTx14 = tolua.cast(sv:getChildByName('info_14_tx'), 'UILabel')
		-- infoTx14:setText(getLocalStringValue('HELP_LEGION_14'))
		-- infoTx14:setPreferredSize(580,0)	
		sv:setClippingEnable(true)
		sv:scrollToTop()
	end)
end