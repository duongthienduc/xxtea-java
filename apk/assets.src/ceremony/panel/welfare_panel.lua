...
function Welfare.enter()
	...
	local function getConf()
		allgiftData = GameData:getArrayData('allgift.dat')
	...
	local function updateMainPanel()
		for i=1,MAX_AWARD do
			...
			elseif allStatus[tostring(i)] == 1 then
				...
				getAllAwardBtns[i]:setFontSize(28)
				getAllAwardBtns[i]:setText(getLocalStringValue('E_STR_ARENA_GOT_REWARD'))
			else 
				getAllAwardBtns[i]:setText('')
				local str = UILabel:create()
				str:setPreferredSize(100,1)
				str:setFontSize(20)
				str:setText(getLocalStringValue('E_STR_GIFTS_DAY_BTN_DESC_7'))
				getAllAwardBtns[i]:setNormalButtonGray(true)
				getAllAwardBtns[i]:setTouchEnable(false)
				getAllAwardBtns[i]:addChild(str)
	...
	local function getAwardRsp(i)
		...
		Message.sendPost('get_all_gift_reward','activity',json.encode(args),function (jsondata)
	...
	local function createPanel()
		...
		sceneObj = SceneObjEx:createObj('panel/whole_people_bg_panel.json','Welfare-in-lua')
	...
	local function getWelfareResponse()
		Message.sendPost('get_all_gift','activity','{}',function (jsondata)
